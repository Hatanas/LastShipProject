...
finalize(), initialize(), execute(), decideNext()はオーバーライドする.
---------------------------------------------------------------------------------------*/
...
public:
	...
	typedef SceneID ID;
	//シーンの遷移方法クラスの基本クラス
	typedef std::unique_ptr<BaseSceneChangeMethod<ID>> ChangeMethod;
	//ChangeMethodFactory::get<遷移方法クラス>(次のシーン識別名)を呼んで次のシーンへの遷移方法クラスを得る事ができる.
	typedef SceneChangeMethodFactory<ID> MethodFactory;
public:
	BaseScene() {}
	virtual ~BaseScene() {}
public:
